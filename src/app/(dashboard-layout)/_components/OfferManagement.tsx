...
export default function OfferManagement({
  ...
  const allowedStatuses = statusOptions.map(
    (o) => o.value
  ) as AdminReviewStatus[];
  const initialStatus = allowedStatuses.includes(
  ...
  const [offer, setOffer] = useState(
    initialOffer || { valuation: '', fees: '', timeline: '' }
  );
      ...
      <div>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
          <div className='space-y-4'>
            <p className='text-sm font-bold text-navy uppercase tracking-wider'>
              Current Status
            </p>
            <div className='w-full'>
              <Select
                ...
                value={status}
                setValue={async (val) => setStatus(val as AdminReviewStatus)}
                variant='light'
              />
            </div>
          </div>
